    ...
    const buildSchoolDetails = (categoryKey: string): SchoolIssueDetail[] => {
        ...
        return schools.map(school => {
            ...
            return {
                ...
                affectedMetrics: affectedMetricsForSchool.map(key => {
                    const challengeText = METRIC_TO_CHALLENGE_MAP[key as string];
                    if (challengeText) return challengeText;
                    
                    // Create a clear challenge description from the field name
                    const fieldName = FIELD_HEBREW_MAP[key as string];
                    if (fieldName) {
                        const parts = fieldName.split(' - ');
                        const specificField = parts[1] || parts[0];
                        return `בית הספר מתמודד עם אתגר בתחום: ${specificField}`;
                    }
                    
                    return `בית הספר מתמודד עם אתגר במדד: ${key}`;
                })
            };
          ...
          issues.map((issue) => (
            <div key={issue.id} className={`border rounded-lg transition-all duration-300 ${ selectedIssues.includes(issue.id) ? 'border-blue-500 bg-blue-50 shadow-lg' : 'border-gray-200 bg-white hover:border-gray-300'}`}>
              ...
              <div className="px-4 md:px-6 py-3 border-t border-gray-200 bg-white/50">
                  <button onClick={(e) => { e.stopPropagation(); handleToggleDetails(issue.id);}} className="flex justify-between items-center w-full text-sm font-semibold text-blue-700 hover:text-blue-800" data-tour="issue-details">
