...
import { Issue, AllInterventionPlans, TierPlan, PlanComponent, TieredSchools, SchoolForAnalysis, Tier2Group, InformationCard } from '../types';
import type { Tier3PlanManager } from '../types';
...
const PlanPrintLayout: React.FC<{ 
    ...
    const InformationCardDisplay: React.FC<{ card: InformationCard }> = ({ card }) => {
        const renderSupervisorSupport = () => {
            const { supervisorSupport } = card;
            if (Array.isArray(supervisorSupport)) {
                return supervisorSupport.map((role, i) => <li key={i}><BoldingFormatter text={role} /></li>);
            }
            if (supervisorSupport && typeof supervisorSupport === 'object' && !Array.isArray(supervisorSupport)) {
                const supportObject = supervisorSupport as { roles?: string[]; activities?: string[] };
                return (
                    <>
                        {supportObject.roles?.map((role, i) => <li key={`role-${i}`}><BoldingFormatter text={role} /></li>)}
                        {supportObject.activities?.map((activity, i) => <li key={`activity-${i}`}><BoldingFormatter text={activity} /></li>)}
                    </>
                );
            }
            return <li><BoldingFormatter text={String(supervisorSupport || '')} /></li>;
        };
                                    ...
                                    Object.entries(plan.tier3.individualPlans).map(([schoolId, individualPlan]) => {
                                        ...
                                        return (
                                            <div key={schoolId} className="mt-3 p-2 border-t border-red-200 page-break-inside-avoid">
                                                ...
                                                {renderPlan(individualPlan as TierPlan)}
                                            </div>
                                        );
                                    })
                                )}
...
// Fix: Added a missing 'return' statement for the main JSX of the InterventionPlanBuilder component to ensure it always returns a ReactNode, resolving a TypeScript compilation error.
                                        ...
                                        ` : Object.entries(plan.tier3.individualPlans).map(([schoolId, individualPlan]) => {
                                            ...
                                            return `
                                                <div style="margin: 10px 0; padding: 10px; border-top: 1px solid #ef4444;">
                                                    ...
                                                    ${renderPlanForWord(individualPlan as TierPlan)}
                                                </div>
                                            `;
                                        }).join('')}
                                    </div>
        ...
        return (
            <div className="bg-gray-50 p-4 sm:p-8 rounded-lg">
                <header className="flex flex-wrap justify-between items-center gap-4 mb-6">
                     <div>
                        <h2 className="text-3xl font-bold text-gray-800">סיכום תוכנית התערבות</h2>
                        <p className="text-gray-600">זהו סיכום של התוכנית/ות שבנית. ניתן להדפיס, להוריד או לחזור לעריכה.</p>
                     </div>
                     <div className="flex items-center gap-2 flex-wrap">
                        <button onClick={() => setCurrentView('build')} className="px-4 py-2 bg-gray-200 text-gray-800 rounded-lg font-semibold hover:bg-gray-300">חזור לעריכה</button>
                        <button onClick={handlePrint} className="px-4 py-2 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 flex items-center gap-2" data-tour="export-options"><Printer size={16}/> הדפס</button>
                        <button onClick={handleDownloadPdf} disabled={isGeneratingPdf} className="px-4 py-2 bg-green-600 text-white rounded-lg font-semibold hover:bg-green-700 flex items-center gap-2 disabled:bg-gray-400" data-tour="export-options">
                            {isGeneratingPdf ? <Loader className="animate-spin" size={16} /> : <Download size={16} />}
                            {isGeneratingPdf ? 'מייצא...' : 'הורד PDF'}
                        </button>
                        <button onClick={handleDownloadWord} className="px-4 py-2 bg-sky-600 text-white rounded-lg font-semibold hover:bg-sky-700 flex items-center gap-2" data-tour="export-options"><Download size={16}/> הורד Word</button>
                     </div>
                </header>

                <div className="mb-4">
                    <label htmlFor="plan-select" className="block text-sm font-medium text-gray-700">הצג תוכנית עבור:</label>
                    <select
                        id="plan-select"
                        value={selectedPlanIssueId || ''}
                        onChange={(e) => setSelectedPlanIssueId(e.target.value)}
                        className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md"
                    >
                        {relevantPlanIssues.map(p => <option key={p.id} value={p.id}>{p.title}</option>)}
                    </select>
                </div>

                <div ref={printLayoutRef} className="bg-white rounded-lg shadow-xl overflow-hidden">
                    <PlanPrintLayout 
                        plans={plans} 
                        issues={issueForPrint} 
                        tieredSchools={tieredSchools} 
                 ...
                 <div className="mt-8 flex justify-between">
                    ...
                    <button onClick={onReset} className="flex items-center gap-2 px-6 py-3 bg-red-600 text-white font-bold rounded-lg shadow-lg hover:bg-red-700 transition-all transform hover:scale-105">
                        <RotateCcw size={18} />
                        התחל תהליך מחדש
                    </button>
                 </div>
        ...
        <div className="bg-white p-6 sm:p-8 rounded-lg shadow-lg">
            <header className="text-center mb-8">
                <h1 className="text-3xl md:text-4xl font-bold text-gray-800">
                    {currentView === 'select' && 'שלב 5: בחירת סוגיה לתכנון'}
                    {currentView === 'build' && `שלב 5: בניית תוכנית התערבות`}
                </h1>
                <p className="text-gray-500 mt-2 text-lg max-w-3xl mx-auto">
                    {currentView === 'select' && 'בחר/י את הסוגיה שעבורה תרצה/י לבנות תוכנית התערבות מפורטת.'}
                    {currentView === 'build' && `להלן תוכנית ההתערבות המוצעת עבור "${selectedIssueForPlan?.title}". ניתן לערוך, להוסיף ולהתאים אותה לצרכים הייחודיים.`}
                </p>
            </header>

            {currentView === 'select' && (
                <div className="space-y-4">
                    {relevantPlanIssues.length > 0 ? relevantPlanIssues.map(p => (
                        <button 
                            key={p.id}
                            onClick={() => {
                                setSelectedPlanIssueId(p.id);
                                setOpenAccordion(p.id);
                                setCurrentView('build');
                            }}
                            className="w-full text-right p-6 bg-gray-50 rounded-lg border border-gray-200 hover:border-blue-500 hover:bg-blue-50 transition-all flex justify-between items-center"
                        >
                            <span className="text-xl font-semibold text-gray-700">{p.title}</span>
                            <ArrowLeft className="text-blue-600" />
                        </button>
                    )) : (
                        <div className="text-center p-8 bg-gray-50 rounded-lg">
                            <h3 className="text-xl font-semibold text-gray-600">לא נבחרו סוגיות</h3>
                            <p className="text-gray-500 mt-2">יש לחזור לשלב הקודם ולבחור לפחות סוגיה אחת להתערבות.</p>
                        </div>
                    )}
                </div>
            )}

            {currentView === 'build' && selectedPlanIssueId && plans[selectedPlanIssueId] && (
                <IssueAccordionItem
                    issueId={selectedPlanIssueId}
                    title={relevantPlanIssues.find(p => p.id === selectedPlanIssueId)?.title || ''}
                    plan={plans[selectedPlanIssueId]}
                    tieredSchools={tieredSchools}
                    allSelectedIssues={selectedIssues}
                    onUpdate={handlePlanUpdate}
                    isOpen={openAccordion === selectedPlanIssueId}
                    onToggle={() => setOpenAccordion(prev => prev === selectedPlanIssueId ? null : selectedPlanIssueId)}
                />
            )}
            ...
            <div className="flex justify-between items-center mt-12 border-t pt-6">
                <button
                    onClick={() => {
                        if (currentView === 'build') setCurrentView('select');
                        else onBack();
                    }}
                ...
                {currentView === 'build' && (
                    <button
                        ...
                        className="flex items-center gap-2 px-6 py-3 bg-red-100 text-red-800 font-semibold rounded-lg shadow-sm hover:bg-red-200 transition-colors"
                    >
                        <RotateCcw size={18} />
                        אפס תוכנית לערכי ברירת מחדל
                    </button>
                )}

                <button
                    disabled={!isComplete}
                    onClick={handleFinishAndCreatePlan}
